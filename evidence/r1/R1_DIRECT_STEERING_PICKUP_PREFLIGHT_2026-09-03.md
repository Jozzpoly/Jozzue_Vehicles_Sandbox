...
| --- | --- | --- |
| Unit/operation regression | PASS | `npm run check`: 42/42 tests, including the R1 validation-hardening tests plus existing E1/V0 suites |
...
| Signed-angle branch cut | PASS | Authored LEFT radius `0.24 m` at `167°`, rack `-0.075 m`: oracle returns `+0.2542416855615702 rad` after signed wrapping, rather than the unwrapped `-6.028943621618016 rad` |
...
- **Branch-cut regression:** the `167°` LEFT specimen at rack `-0.075 m` now returns `+0.2542416855615702 rad` from the normalized signed difference; the prior unwrapped value was `-6.028943621618016 rad` and could falsely remain under the permissive oracle's `VALID` envelope.
...
- This micro-hardening touches only analytical geometry and its regression test, so the rendered Browser suite was not repeated; the prior canonical 12/12 rendered result remains the applicable UI evidence.
