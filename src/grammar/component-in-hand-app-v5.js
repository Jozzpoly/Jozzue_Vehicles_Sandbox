...
const WORLD_PREVIEW_RADIUS = 0.34;
const SCREEN_PREVIEW_RADIUS = 34;
const WORLD_UP = new THREE.Vector3(0, 1, 0);
const app = document.querySelector("#app");
if (!app) throw new Error("Family C V5 requires #app");

document.title = "JV Family C · Spatial Compass";
app.innerHTML = `
<style>
:root{color-scheme:dark;font-family:Inter,system-ui,sans-serif}*{box-sizing:border-box}body{margin:0;overflow:hidden;background:#080c11;color:#edf3f8}.stage{position:relative;width:100vw;height:100vh}canvas{display:block;width:100%;height:100%;touch-action:none}.panel{position:absolute;top:16px;left:16px;width:328px;padding:13px 14px;border:1px solid #40515f;border-radius:12px;background:rgba(8,13,19,.91);backdrop-filter:blur(7px);box-shadow:0 14px 34px #0006}.eyebrow{margin:0 0 4px;color:#98adbd;font-size:10px;letter-spacing:.14em}h1{margin:0;font-size:21px}.warn{margin:8px 0;padding:7px 9px;border:1px solid #745a3d;border-radius:8px;color:#e8c491;background:#6646222e;font-size:10px;font-weight:700}.sub,.hint{font-size:11px;line-height:1.42;color:#a5b5c2}.hint{font-size:10px;color:#7f93a3}.row{display:grid;grid-template-columns:82px 1fr;gap:6px;padding:3px 0;font-size:10px;color:#8395a4}.row output{color:#e1e9ef}.buttons{display:flex;gap:6px;margin-top:9px}button{border:1px solid #506678;border-radius:8px;background:#131d27;color:#eaf2f8;padding:7px 9px;font:11px system-ui;cursor:pointer}button:disabled{opacity:.35}.status{margin-top:9px;min-height:48px;padding:8px 9px;border:1px solid #2c3e4c;border-radius:8px;background:#0d151d;color:#bdcad5;font-size:11px;line-height:1.35}.legend{display:flex;gap:9px;margin-top:8px;color:#95a7b5;font-size:9px}.legend b{font-weight:800}.span{color:#f0bc58}.lift{color:#60d9d0}.side{color:#b995ef}.racknote{position:absolute;right:24px;bottom:20px;padding:8px 10px;border:1px solid #60717e;border-radius:9px;background:#080d13d9;color:#d3dde5;font-size:10px;pointer-events:none}
</style>
<main class="stage"><canvas data-testid="family-c-canvas"></canvas><aside class="panel"><p class="eyebrow">NEXTGEN JV · FAMILY C · SPATIAL COMPASS V0.1</p><h1>Take it. Fit it. Rework it.</h1><div class="warn">INTERACTION SPIKE · NO PHYSICS CLAIM</div><p class="sub">Direct eye drag follows the current view. Point at a visible compatible mount and the 3D snap becomes an explicit preview; release commits it. The optional mini-compass speaks the damper's own frame instead of generic XYZ.</p><div class="row"><span>selected</span><output data-testid="selected">none</output></div><div class="row"><span>active eye</span><output data-testid="active-end">—</output></div><div class="row"><span>parts</span><output data-testid="count">0</output></div><div class="row"><span>donor</span><output data-testid="donor">loading…</output></div><div class="legend"><b class="span">SPAN</b><span>along part</span><b class="lift">LIFT</b><span>gravity-plane</span><b class="side">SIDE</b><span>lateral</span></div><div class="buttons"><button data-testid="undo">Undo</button><button data-testid="delete" disabled>Delete part</button><button data-testid="clear">Clear</button></div><div class="status" data-testid="status">Loading real Asset_Dumper…</div><p class="hint">Drag eye = free current-view move. Petals = constrained semantic move. Empty-space drag orbits while keeping selection. Pale halo + tether means “release here to attach”. Weird finite placements remain allowed.</p></aside><div class="racknote">PART RACK · real Asset_Dumper</div></main>`;
...
function cylinder(r,c,opacity=1){return new THREE.Mesh(new THREE.CylinderGeometry(r,r,1,14),new THREE.MeshStandardMaterial({color:c,roughness:.42,metalness:.2,transparent:opacity<1,opacity,depthWrite:opacity>.4}))}
function nearestSocket(pos,radius=WORLD_PREVIEW_RADIUS){let best=null,dist=radius;for(const s of sockets){const d=pos.distanceTo(s.p);if(d<dist){best=s;dist=d}}return best}
...
const snapHalo=new THREE.Mesh(new THREE.SphereGeometry(.092,20,14),new THREE.MeshBasicMaterial({color:0xd9f4ff,transparent:true,opacity:.48,wireframe:true,depthWrite:false}));snapHalo.visible=false;scene.add(snapHalo);const snapLine=cylinder(.009,0xd9f4ff,.38);snapLine.visible=false;scene.add(snapLine);
...
function restore(h){parts=h.parts.map(p=>({id:p.id,a:{p:new THREE.Vector3(...p.a),s:p.as},b:{p:new THREE.Vector3(...p.b),s:p.bs}}));selectedId=h.selectedId&&parts.some(p=>p.id===h.selectedId)?h.selectedId:null;activeEnd=h.activeEnd??"a";clearPreview();sync();refresh();render()}
...
function updateGizmo(){const p=selectedId?part(selectedId):null;if(!p){gizmo.root.visible=false;return}const frame=componentFrame(p,activeEnd);const radius=clamp(camera.position.distanceTo(frame.origin)*.055,.14,.25);gizmo.root.visible=true;for(const name of["span","lift","side"]){const tip=frame.origin.clone().addScaledVector(frame[name],radius);segment(gizmo.stems[name],frame.origin,tip);gizmo.handles[name].position.copy(tip);gizmo.handles[name].scale.setScalar(clamp(radius/.18,.82,1.3))}}
function clearPreview(){previewSocket=null;snapHalo.visible=false;snapLine.visible=false}
function showPreview(anchor,socket){previewSocket=socket;if(!socket){snapHalo.visible=false;snapLine.visible=false;return}snapHalo.visible=true;snapHalo.position.copy(socket.p);snapLine.visible=true;segment(snapLine,anchor,socket.p)}
function worldPreview(anchor){showPreview(anchor,nearestSocket(anchor))}
function commitSnap(end,preferred=null){const best=preferred??nearestSocket(end.p,SNAP_RADIUS);end.s=best?.id??null;if(best)end.p.copy(best.p);return best}
function refresh(){const p=selectedId?part(selectedId):null;selectedOut.textContent=p?`damper · A ${p.a.s??"free"} · B ${p.b.s??"free"}`:"none";activeOut.textContent=p?activeEnd.toUpperCase():"—";countOut.textContent=String(parts.length);donorOut.textContent=donorReady?"READY · Asset_Dumper":"loading…";undoBtn.disabled=!history.length;deleteBtn.disabled=!p;publish()}
...
function client(p){const q=p.clone().project(camera),r=canvas.getBoundingClientRect();return{x:r.left+(q.x*.5+.5)*r.width,y:r.top+(-q.y*.5+.5)*r.height,z:q.z}}
function screenSocket(x,y,radius=SCREEN_PREVIEW_RADIUS){let best=null,dist=radius;for(const s of sockets){const q=client(s.p);if(q.z<-1||q.z>1)continue;const d=Math.hypot(q.x-x,q.y-y);if(d<dist){best=s;dist=d}}return best}
...
function directPreview(anchor,x,y){showPreview(anchor,screenSocket(x,y)??nearestSocket(anchor))}
function capture(e){orbit.enabled=false;canvas.setPointerCapture(e.pointerId);e.preventDefault();e.stopPropagation()}
function beginDirect(e,pid,end){const p=part(pid);if(!p)return;push();selectedId=pid;activeEnd=end;p[end].s=null;const start=p[end].p.clone();drag={pointerId:e.pointerId,pid,end,mode:"direct",plane:viewPlane(start),start};capture(e);directPreview(start,e.clientX,e.clientY);refresh();render();status(`Eye ${end.toUpperCase()} · free current-view move. Hover a mount for an explicit 3D snap preview.`)}
function beginAxis(e,pid,end,axisName){const p=part(pid);if(!p)return;push();selectedId=pid;activeEnd=end;p[end].s=null;const frame=componentFrame(p,end),axis=frame[axisName].clone(),screen=axisScreen(frame.origin,axis);drag={pointerId:e.pointerId,pid,end,mode:"axis",axisName,axis,start:p[end].p.clone(),startPointer:{x:e.clientX,y:e.clientY},screen};capture(e);worldPreview(p[end].p);refresh();render();status(`${axisName.toUpperCase()} · constrained in the damper frame. Direct eye drag remains available for free view-plane motion.`)}
function beginCarry(e,pid,recordHistory=true){const p=part(pid);if(!p)return;if(recordHistory)push();selectedId=pid;p.a.s=p.b.s=null;const mid=p.a.p.clone().add(p.b.p).multiplyScalar(.5),plane=viewPlane(mid),grab=pointOnPlane(e.clientX,e.clientY,plane,mid);drag={pointerId:e.pointerId,pid,mode:"carry",plane,last:grab};capture(e);clearPreview();render();status("Whole free damper in hand · current-view plane. Release may attach its nearer eye.")}
...
canvas.addEventListener("pointerdown",e=>{if(e.button!==0)return;const gh=hit(e.clientX,e.clientY,Object.values(gizmo.handles));if(gh?.object.userData.gizmoAxis&&selectedId){beginAxis(e,selectedId,activeEnd,gh.object.userData.gizmoAxis);return}const hh=hit(e.clientX,e.clientY,[...visuals.values()].flatMap(v=>[v.a,v.b]));if(hh?.object.userData.end){beginDirect(e,hh.object.userData.partId,hh.object.userData.end);return}const rh=hit(e.clientX,e.clientY,[rackPicker]);if(rh?.object.userData.rack){if(!donorReady){status("Damper is still loading.");return}const rackMid=rack.localToWorld(rackA.clone().add(rackB).multiplyScalar(.5)),plane=viewPlane(rackMid),at=pointOnPlane(e.clientX,e.clientY,plane,rackMid);const p=spawn(at);beginCarry(e,p.id,false);return}const bh=hit(e.clientX,e.clientY,[...visuals.values()].map(v=>v.picker));const id=bh?.object.userData.partId;if(id){selectPart(id);const p=part(id);if(!p.a.s&&!p.b.s)beginCarry(e,id,true);else{status("Damper selected. Drag either eye, or use the active eye's SPAN / LIFT / SIDE petals.");e.preventDefault();e.stopPropagation()}return}
// Empty-space LMB remains OrbitControls so camera changes do not destroy selection.
});

canvas.addEventListener("pointermove",e=>{if(!drag||drag.pointerId!==e.pointerId)return;const p=part(drag.pid);if(!p)return;if(drag.mode==="carry"){const w=pointOnPlane(e.clientX,e.clientY,drag.plane,drag.last),d=w.clone().sub(drag.last);p.a.p.add(d);p.b.p.add(d);drag.last.copy(w)}else if(drag.mode==="direct"){p[drag.end].p.copy(pointOnPlane(e.clientX,e.clientY,drag.plane,drag.start));directPreview(p[drag.end].p,e.clientX,e.clientY)}else{const dx=e.clientX-drag.startPointer.x,dy=e.clientY-drag.startPointer.y,projected=dx*drag.screen.ux+dy*drag.screen.uy;const scalar=drag.screen.pixelsPerUnit>8?projected/drag.screen.pixelsPerUnit:-dy/180;p[drag.end].p.copy(drag.start).addScaledVector(drag.axis,scalar);worldPreview(p[drag.end].p)}sync();refresh();render()});
function finish(e){if(!drag||drag.pointerId!==e.pointerId)return;const p=part(drag.pid);let snapped=null;if(p){if(drag.mode==="carry"){const da=Math.min(...sockets.map(s=>p.a.p.distanceTo(s.p))),db=Math.min(...sockets.map(s=>p.b.p.distanceTo(s.p)));snapped=commitSnap(da<=db?p.a:p.b)}else{const preferred=drag.mode==="direct"?previewSocket:null;snapped=commitSnap(p[drag.end],preferred)}}if(canvas.hasPointerCapture(e.pointerId))canvas.releasePointerCapture(e.pointerId);orbit.enabled=true;drag=null;clearPreview();if(p){const n=[p.a.s,p.b.s].filter(Boolean).length;status(snapped?`Snapped to ${snapped.id}. ${n===2?"Both eyes attached; topology stays editable.":"One eye attached; keep fitting or repositioning."}`:n===0?"Part is free in space.":"Placement kept unsnapped. One eye remains attached.")}sync();refresh();render()}
...
async function load(){try{const gltf=await new GLTFLoader().loadAsync(DONOR_URL);donorTemplate=gltf.scene;donorReady=true;rackDonor=cloneSkeleton(donorTemplate);const scaled=bindRep4DamperDonorAtPhysicalRestLength(rackDonor,.5);applyC1DamperBetween(scaled.binding,rackA,rackB);rack.add(rackDonor);refresh();render();status("Ready. Direct movement follows the view; visible mounts become explicit 3D release targets.")}catch(err){donorOut.textContent="ERROR";status(`Donor load failed: ${err instanceof Error?err.message:String(err)}`);render()}}
