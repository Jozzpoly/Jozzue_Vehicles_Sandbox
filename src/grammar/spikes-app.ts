...
if (rootCandidate === null) throw new Error("Construction Grammar Spikes V0 requires #app.");
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .grammar { width: 100vw; height: 100vh; position: relative; }
  .grammar canvas { width: 100%; height: 100%; display: block; touch-action: none; }
  .panel { position: absolute; top: 16px; left: 16px; width: min(430px, calc(100vw - 32px)); padding: 14px 15px; border: 1px solid #3f5162; border-radius: 12px; background: rgba(8,13,19,.92); backdrop-filter: blur(8px); box-shadow: 0 14px 34px rgba(0,0,0,.34); }
  .eyebrow { margin: 0 0 4px; color: #9cb2c5; font-size: 10px; letter-spacing: .14em; text-transform: uppercase; }
  h1 { margin: 0; font-size: 20px; line-height: 1.15; }
  .warning { margin: 8px 0 10px; padding: 8px 10px; border: 1px solid #805b35; border-radius: 8px; color: #efc38c; background: rgba(107,70,30,.18); font-size: 11px; font-weight: 650; letter-spacing: .03em; }
  .sub { margin: 7px 0 10px; color: #aabac8; font-size: 12px; line-height: 1.4; }
  .section { margin-top: 10px; padding-top: 10px; border-top: 1px solid #273543; }
  ...
  .status { margin-top: 8px; padding: 8px 9px; border: 1px solid #2d4051; border-radius: 8px; background: #0d151d; color: #b8c8d5; font-size: 11px; line-height: 1.38; }
  .status strong { color: #edf4fa; }
  .row { display: grid; grid-template-columns: 112px 1fr; gap: 7px; padding: 3px 0; color: #9badbd; font-size: 11px; }
  ...
  .mode-note { min-height: 31px; margin-top: 6px; color: #9fb1c0; font-size: 11px; line-height: 1.35; }
</style>
<main class="grammar">
  ...
  <aside class="panel">
    ...
    <p class="sub">Same corner, two grammars. Build arbitrary connection graphs, delete them, reconnect endpoints and compare which interaction deserves real-physics integration later.</p>
    ...
    <div class="section">
      <div class="label">Relation component</div>
      <div class="buttons">
    ...
    <p class="legend">Small cyan sockets belong to the chassis; orange sockets belong to the hub/upright reference. Empty-space drag orbits the camera. Select a relation to reconnect either endpoint. <span class="kbd">Ctrl+Z</span> undoes topology edits. Weird finite graphs are intentionally allowed.</p>
  </aside>
</main>`;
...
const sockets: readonly SocketDef[] = Object.freeze([
  ...
  { id: "c-damper", label: "chassis damper mount", group: "chassis", position: new THREE.Vector3(-0.18, 0.52, 0.02) },
...
const socketById = new Map(sockets.map((socket) => [socket.id, socket] as const));

function socket(id: string): SocketDef {
  const value = socketById.get(id);
  if (!value) throw new Error(`Unknown construction socket ${id}`);
  return value;
}

const exampleRelations = (): RelationState[] => [
...
let relations: RelationState[] = exampleRelations();
...
camera.position.set(2.4, 1.35, 2.65);
...
scene.add(keyLight);
const rimLight = new THREE.DirectionalLight(0x8dbde3, 1.2);
...
const chassis = new THREE.Mesh(
  new THREE.BoxGeometry(0.28, 1.18, 0.95),
...
for (const def of sockets) {
  const color = def.group === "chassis" ? 0x5bd5e8 : 0xffa84d;
...
function segmentMesh(radius: number, color: number, opacity = 1): THREE.Mesh<THREE.CylinderGeometry, THREE.MeshStandardMaterial> {
  const mesh = new THREE.Mesh(
    new THREE.CylinderGeometry(radius, radius, 1, 16),
    new THREE.MeshStandardMaterial({ color, roughness: 0.42, metalness: 0.3, transparent: opacity < 1, opacity }),
  );
  mesh.castShadow = true;
  return mesh;
}
...
const selectionMaterial = new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.18, depthWrite: false });

function removeRelationVisuals(): void {
  for (const visual of relationVisuals.values()) {
    scene.remove(visual.root);
    scene.remove(visual.selection);
  }
  relationVisuals.clear();
}

function makeSelectionHalo(a: THREE.Vector3, b: THREE.Vector3): THREE.Mesh {
  const halo = new THREE.Mesh(new THREE.CylinderGeometry(0.055, 0.055, 1, 14), selectionMaterial);
  setSegment(halo, a, b);
  halo.visible = false;
  halo.renderOrder = 12;
  return halo;
...
function tagRelation(object: THREE.Object3D, relationId: string): void {
  object.traverse((child) => { child.userData.relationId = relationId; });
}

function rebuildRelationVisuals(): void {
  removeRelationVisuals();
  for (const relation of relations) {
    const a = socket(relation.a).position;
    const b = socket(relation.b).position;
    const halo = makeSelectionHalo(a, b);
    halo.visible = relation.id === selectedRelationId;
    scene.add(halo);

    if (relation.kind === "damper" && donorTemplate !== null) {
      const donor = donorTemplate.clone(true);
      const scaled = bindRep4DamperDonorAtPhysicalRestLength(donor, DONOR_REFERENCE_LENGTH);
      applyC1DamperBetween(scaled.binding, a, b);
      tagRelation(donor, relation.id);
      scene.add(donor);
      relationVisuals.set(relation.id, { root: donor, selection: halo, damperBinding: scaled.binding });
    } else {
      const color = relation.kind === "damper" ? 0xb78bd4 : 0xaebdca;
      const mesh = segmentMesh(relation.kind === "damper" ? 0.032 : 0.025, color);
      setSegment(mesh, a, b);
      mesh.userData.relationId = relation.id;
      scene.add(mesh);
      relationVisuals.set(relation.id, { root: mesh, selection: halo, damperBinding: null });
    }
  }
}

const previewGeometry = new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(), new THREE.Vector3()]);
const previewLine = new THREE.Line(previewGeometry, new THREE.LineBasicMaterial({ color: 0xe9f4ff, transparent: true, opacity: 0.75 }));
...
function mutateRelations(next: RelationState[]): void {
  pushHistory();
  relations = next;
...
function addRelation(a: string, b: string): void {
  if (a === b) {
    setStatus("Choose two different sockets. Self-links are not useful for this spike.");
    return;
  }
  const id = `r-${nextRelationIndex++}`;
  mutateRelations([...relations, { id, kind: activeKind, a, b }]);
...
function deleteSelected(): void {
  ...
  mutateRelations(relations.filter((relation) => relation.id !== selectedRelationId));
  setStatus("Relation removed. Undo is available.");
}
...
function reconnectSelected(endpoint: EndpointName, targetSocket: string): void {
  ...
  const next = relations.map((relation) => relation.id === selected.id
...
function resetForMode(nextMode: GrammarMode): void {
  ...
  setStatus(nextMode === "drag"
    ? "Drag from one socket to another to create the selected relation component."
    : "Choose a component, click endpoint A, then click endpoint B.");
}
...
function setKind(kind: RelationKind): void {
  ...
  setStatus(kind === "damper"
    ? "Real donor damper selected for the next new relation."
    : "Adaptive point-to-point link selected for the next new relation.");
}
...
function refreshSocketAppearance(): void {
  for (const def of sockets) {
    ...
    const isPending = def.id === pendingSocketId;
    const material = mesh.material as THREE.MeshStandardMaterial;
    mesh.scale.setScalar(isPending ? 1.45 : 1);
    material.emissiveIntensity = isPending ? 0.75 : 0.22;
  }
}
...
function refreshUi(): void {
  ...
  modeNote.textContent = mode === "drag"
    ? "Gesture-first: relation intent is expressed by dragging directly between physical connection points."
    : "Tool-first: choose the component first, then explicitly nominate its two endpoints.";
  ...
  donorOutput.textContent = donorReady ? "READY · real Asset_Dumper" : "loading… fallback visual until ready";
...
clearButton.addEventListener("click", () => {
  mutateRelations([]);
  setStatus("Construction cleared. Build any relation graph you want.");
});
exampleButton.addEventListener("click", () => {
  mutateRelations(exampleRelations());
  setStatus("Restored the same small example corner for comparison.");
});
...
function relationHit(clientX: number, clientY: number): string | null {
  pointerToNdc(clientX, clientY);
  raycaster.setFromCamera(pointerNdc, camera);
  const roots = [...relationVisuals.values()].map((visual) => visual.root);
...
function pointerWorld(clientX: number, clientY: number): THREE.Vector3 {
  ...
  return raycaster.ray.intersectPlane(constructionPlane, point) ?? new THREE.Vector3();
}
...
canvas.addEventListener("pointerdown", (event) => {
  ...
  if (hitSocketId !== null && mode === "tool") {
    if (pendingSocketId === null) {
      ...
      setStatus(`Created ${activeKind} relation. Select it to reconnect or delete.`);
    }
    event.preventDefault();
    event.stopPropagation();
    return;
  ...
  if (hitRelationId !== null) {
    ...
    rebuildRelationVisuals();
    refreshUi();
    setStatus("Relation selected. Reconnect either endpoint, delete it, or continue building.");
    event.preventDefault();
    event.stopPropagation();
  }
});
...
function finishDrag(event: PointerEvent): void {
  ...
  dragPointerId = -1;
  if (target !== null) {
    addRelation(start, target);
    setStatus(`Created ${activeKind} relation by direct drag.`);
  } else {
    setStatus("Connection cancelled — release on a socket to create a relation.");
  }
}

canvas.addEventListener("pointerup", finishDrag);
canvas.addEventListener("pointercancel", (event) => {
  if (dragStartSocketId !== null) {
    dragStartSocketId = null;
    hidePreview();
    orbit.enabled = true;
  }
...
function publishEvidence(): void {
  ...
  root.dataset.kind = activeKind;
  root.dataset.relationCount = String(relations.length);
  root.dataset.selectedRelation = selectedRelationId ?? "none";
  root.dataset.pendingSocket = pendingSocketId ?? "none";
  root.dataset.reconnectEndpoint = reconnectEndpoint ?? "none";
  root.dataset.donorReady = String(donorReady);
  root.dataset.canUndo = String(history.length > 0);
  root.dataset.interactionCount = String(interactionCount);
  root.dataset.relations = JSON.stringify(relations.map(({ id, kind, a, b }) => ({ id, kind, a, b })));
}
...
function publishScreenEvidence(): void {
  for (const def of sockets) {
    ...
    root.dataset[`${key}ScreenX`] = String(screen.x);
    root.dataset[`${key}ScreenY`] = String(screen.y);
  }
...
async function prepareDonor(): Promise<void> {
  try {
    ...
    setStatus("Damper donor failed to load. Generic link grammar remains usable, but do not use this checkpoint for donor comparison.");
    publishEvidence();
  }
}
...
rebuildRelationVisuals();
refreshUi();
setStatus("Loading real damper donor… You can already explore generic link topology.");
void prepareDonor();
requestAnimationFrame(animate);
